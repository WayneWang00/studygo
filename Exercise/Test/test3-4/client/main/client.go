...
//关闭信息显示通道
var stopMessage = make(chan bool, 1)
...
func showMessage(conn net.Conn) {
	...
	for {
		...
		if cmd == S2C_BROADCAST_ENTER_ROOM {
			...
			if !ok {
				fmt.Println("接受进入玩家信息失败")
			}
			fmt.Println("玩家" + loginInfoIn.Name + "进入房间")
		}
		if cmd == S2C_BROADCAST_OUT_ROOM {
			loginInfoOut, ok := message.(msg.LoginInfo)
			if !ok {
				fmt.Println("接受退出玩家信息失败")
			}
			fmt.Println("玩家" + loginInfoOut.Name + "退出房间")
		}
		if cmd == S2C_BROADCAST_OFFLINE {
			loginInfoOff, ok := message.(msg.LoginInfo)
			if !ok {
				fmt.Println("接受退出玩家信息失败")
			}
			fmt.Println("玩家" + loginInfoOff.Name + "离线")
		}
