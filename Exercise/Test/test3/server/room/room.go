...
//查询离线玩家信息
func (this *ChatRoom) GetUserInfoFromSql(onlineUsers []User) []msg.UserInfo {
...
func (this *ChatRoom) UpdateUserTimeInfo(user *User) {
	total := user.TotalTime + time.Now().Second() - user.LoginTime.Second()
	mysql.UpDateUserTimeInfo(user.Uid, &mysql.UserInfo{
		Status:        "offline",
		OfflineTime:   time.Now().String(),
		OnlineTime:    total,
		DefaultRoomId: user.Room.Rid})
}
