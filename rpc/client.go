...
func main() {
	...
	for i := 0; i < 200000; i++ {
		...
		//				response := client.SendAndRecvRespRpcMsg("JungleArith.Add", reqs[j], 5000, 0)
		...
		//				}
		//				if i%20 == 0 {
		//					tsli := make([]byte, 250000, 250000)
		//					//					for k, _ := range tsli {
		//					//						tsli[k] = 1
		//					//					}
		//					response := client.SendAndRecvRespRpcMsg("JungleArith.Add", tsli, 5000, 0)
		//					plog.Debug("response.ReturnCode is:", response.ReturnCode)
		//				}
		//			}(j)
		//		}

		//rpc 点对组无响应测试
		//client.SendNoRespRpcMsgGroup("JungleArith.Add", reqs[0], []int64{})

		//rpc 点对点无响应测试
		err := client.SendNoRespRpcMsg("JungleArith.Add", reqs[0], 0)
		plog.Debug("the SendNoRespRpcMsg is :", err)
